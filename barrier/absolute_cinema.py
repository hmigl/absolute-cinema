...
def main():
    ...
    def timeout_handler():
        print("Timeout reached! Signaling players to stop...")
        stop_event.set()
        for row in theater.barriers:
            for barrier in row:
                if barrier:
                    barrier.interrupt()

    timer = threading.Timer(15, timeout_handler)
    timer.start()

    with ThreadPoolExecutor(max_workers=NUMBER_OF_PLAYERS) as executor:
        futures = [executor.submit(player.move, stop_event) for player in players]
